...
<head>
  ...
  <style>
    :root { color-scheme: light; --bg: #f6f7f9; --panel: #ffffff; --text: #17202a; --muted: #687386; --line: #d9dee7; --green: #087f5b; --red: #c92a2a; --blue: #1c5d99; }
    * { box-sizing: border-box; }
    ...
    @media (max-width: 900px) { header, main { padding: 16px; } .metrics, .two { grid-template-columns: 1fr; } }
  </style>
</head>
<body>
  <header>
  ...
  <main class="grid">
    ...
    <section><h2>Price And Portfolio</h2><canvas id="chart" width="1100" height="320"></canvas></section>
    <div class="grid two">
      <section><h2>Open Batches</h2><div class="scroll"><table><thead><tr><th>ID</th><th>Qty</th><th>Avg Price</th><th>P/L</th><th>Created</th><th>Buys</th></tr></thead><tbody>${openBatches.slice().reverse().map((batch) => { const pnl = batch.quantity * (lastPrice - batch.averagePrice); return `<tr><td>${escapeHtml(batch.id)}</td><td>${fmt(batch.quantity, 4)}</td><td>${money(batch.averagePrice)}</td><td class="${pnl >= 0 ? "pos" : "neg"}">${money(pnl)}</td><td>${escapeHtml(batch.createdAt)}</td><td>${(batch.buys || []).length}</td></tr>`; }).join("")}</tbody></table></div></section>
      <section><h2>Recent Orders</h2><div class="scroll"><table><thead><tr><th>Time</th><th>Kind</th><th>Side</th><th>Qty</th><th>Price</th></tr></thead><tbody>${recentOrders.map((order) => `<tr><td>${escapeHtml(order.at)}</td><td>${escapeHtml(order.kind || "-")}</td><td>${escapeHtml(order.side || "-")}</td><td>${fmt(order.quantity || 0, 4)}</td><td>${order.price ? money(order.price) : "-"}</td></tr>`).join("")}</tbody></table></div></section>
    </div>
  </main>
  <script>
    ...
    drawSeries(data.map(d => d.price), "#1c5d99");
    drawSeries(data.map(d => d.total), "#087f5b");
    ctx.fillStyle = "#1c5d99"; ctx.fillText("Price", pad, 18);
    ctx.fillStyle = "#087f5b"; ctx.fillText("Portfolio value", pad + 60, 18);
    function drawSeries(values, color) { const clean = values.filter(v => Number.isFinite(v)); if (clean.length < 2) return; const min = Math.min(...clean); const max = Math.max(...clean); const span = max - min || 1; ctx.strokeStyle = color; ctx.lineWidth = 2; ctx.beginPath(); values.forEach((value, index) => { const x = pad + ((w - pad * 2) * index) / Math.max(1, values.length - 1); const y = h - pad - ((value - min) / span) * (h - pad * 2); if (index === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); }); ctx.stroke(); }
  </script>
</body>
</html>`;
}
