...
- protected API key rotation per pair: replace `CCOM_API_KEY` and `CCOM_API_SECRET` without showing old or new values back in the UI, with `.env` backup, service restart and optional dry-run safety switch,
